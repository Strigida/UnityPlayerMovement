	...
	void Update () {
		...
		switch (moveType)
        {
            case MoveType.VectorDirect:
                //TODOL write code that gets input from player and assigns it to position.x and position.z with speed modifier
            ...
            case MoveType.RotationDirect:
                ...
                //the Quaternion.Euluer function.  Then, modify the transform's rotation and position with speed and rotation speed
                //DONE
                ...
                Quaternion rotationDirect = Quaternion.Euler(0f, rotation, 0f);
                transform.rotation *= rotationDirect;
    ...
    private void FixedUpdate()
    {
        switch (moveType)
        {
            ...
            case MoveType.RigidBodyMouse:
                ...
                //TODO: using stopping distance and Vector3.Distance, use MoveRotation and MovePosition and lerpRot and
                //goal variables to move the rigid body towards the mouse position
                //Reply to TODO: I'm not sure what is being asked, here? rigid body already moves toward mouse position with current code.
    ...
    public void GetMousePosition()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if(Physics.Raycast(ray, out hit, 10000, layerMask, QueryTriggerInteraction.Ignore))
            {
                ...
                //Set rot global variable using the transform.position and the goal somehow (try Vector arithmetic and
                // the Quaternion.LookRotation function)
                //DONE
